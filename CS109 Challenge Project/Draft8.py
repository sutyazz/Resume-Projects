...
# -----------------------------
time_step = 1  # 1 minute per frame
...
def get_traffic_risk(planned_start_time, region="Palo Alto"):
    ...
    # Convert user start time (in minutes) back to hours
...
def get_active_traffic_flights(user_start_time, trip_duration):
    """
    Returns traffic flights that are either already in progress or will start before the user's
    estimated trip (user_start_time + trip_duration) finishes.
    """
    active_flights = []
    for idx, row in traffic_df.iterrows():
        flight_start = time_str_to_float(row["Time of Travel"]) * 60  # convert hours to minutes
        duration = row["Duration (min)"]  # in minutes
        flight_end = flight_start + duration
        # Include flight if it is in progress (flight_end >= user_start_time)
        # or if it starts within the user's trip duration (flight_start <= user_start_time + trip_duration)
...
def simulate():
    ...
    # Use the user's estimated trip duration from the optimized flight for filtering active flights.
