...
def get_traffic_risk(planned_start_time, region="Palo Alto"):
    ...
    planned_hour = planned_start_time / 60.0  # convert minutes back to hours for comparison
...
class FlightPath:
    def __init__(self, flight_id, start_time, start_loc, dest_loc, cruising_altitude):
        ...
        self.start_time = start_time  # In minutes (converted from user input)
        ...
        self.times = None
        self.metadata = {}
        # Set helicopter speed: 95 mph converted to m/min
        # 1 mph ≈ 0.44704 m/s, so 95 mph ≈ 95 * 0.44704 m/s ≈ 42.47 m/s ≈ 42.47*60 ≈ 2548 m/min
    ...
    def generate_path(self):
        ...
        # Travel time in minutes = total_distance (m) / speed (m/min)
...
def get_active_traffic_flights(user_start_time):
    ...
    for idx, row in traffic_df.iterrows():
        ...
        flight_end = flight_start + row["Duration (min)"]
