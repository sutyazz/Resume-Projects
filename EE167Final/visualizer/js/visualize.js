...
function updatePose() {
	...
	if ( stateController.visualizationMode == 0 ) {
		...
		transformBaseStationToWorld( stateController.board.position, stateController.board.quaternion,
			stateController.lighthouse.quaternion, scene.board.position, scene.board.quaternion );
...
function transformBaseStationToWorld( bsPosition, bsQuaternion, bsPitchRollQuaternion, worldPosition, worldQuaternion ) {

	var transB = new THREE.Matrix4().makeTranslation( bsPosition.x, bsPosition.y, bsPosition.z );
	var rotB = new THREE.Matrix4().makeRotationFromQuaternion( bsQuaternion );
	var pitchRoll = new THREE.Matrix4().makeRotationFromQuaternion( bsPitchRollQuaternion );

	var T = new THREE.Matrix4().multiply( pitchRoll ).multiply( transB ).multiply( rotB );

	var scale = new THREE.Vector3();
	T.decompose( worldPosition, worldQuaternion, scale );
