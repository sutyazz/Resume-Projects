...
var StateController = function (dispParams) {
	...
	this.state = {
		...
		// The IMU quaternion is used to rotate the model in the scene.
		accX: 0,
		accY: 0,
		accZ: 0,
		baseStationPitch: 0,
	...
	function initWebSocket() {
		...
		socket.onclose = function () {
			...
			socket.send(closeMsg); //sutyazz addition, just commented out this line

			console.log(closeMsg);

			_this.state.connectionMsg = "Lost...";

			socket = null; //sutyazz addition, just commented out this line

			//try to reconnect in 1s
			setTimeout(initWebSocket, 1000); //sutyazz addition, just commented out this line
		...
		socket.onmessage = function (imu) {
			...
			if (data[0] == "QC") {
				...
				// Parse quaternion (gyro‐integrated or fused on the Teensy):
				...
				// Now parse the raw accelerometer (m/s²) from the Teensy:
				_this.state.accX = Number(data[1]);
				_this.state.accY = Number(data[2]);
				_this.state.accZ = Number(data[3]);
			} else if (data[0] == "PS") {
			...
			/*
			// ── Run the complementary filter whenever we have both a quaternion and a fresh accel reading ──
			// Only run it if accX/Y/Z is nonzero (or you can check a flag so it doesn’t run before the first "AG" arrives).
			if (Math.abs(_this.state.accX) > 1e-6 ||
				Math.abs(_this.state.accY) > 1e-6 ||
				Math.abs(_this.state.accZ) > 1e-6) {
				// Build the “gravity” vector from the last accel sample:
				...
				// Compute the accel‐only quaternion that maps body‐Z (0,1,0) → g:
				const accelQuat = new THREE.Quaternion().setFromUnitVectors(
					new THREE.Vector3(0, 0, 1),  // body’s “forward/down” axis
					g
				);

				// Blend (slerp) 98% of the current IMU quaternion toward the accel‐only quat:
				const alpha = 0.98;
				_this.state.imuQuaternion.slerp(accelQuat, 1 - alpha).normalize();
			}*/
