...
var StandardRenderer = function (webglRenderer, teapots, dispParams) {
	...
	// Set the camera's position and orientation addition for adding blade tracking
	window.threeCamera = camera;
	console.log("Exposed Three.js camera as window.threeCamera:", window.threeCamera);
	...
	// expose teapotScene so we can add the blade from outside
	this.teapotScene = teapotScene; //sutyazz addition

	// Create a scene that has only grid for unwarp-parameter search
	var gridScene = new THREE.Scene();

	// expose teapotScene so we can add the blade from outside
	this.gridScene = gridScene; //sutyazz addition
	...
	var floor_length = 10000;

	var sqGrid = new THREE.GridHelper(floor_width, 30, "gray", "gray");
	var sqGrid1 = new THREE.GridHelper(floor_width, 25, "gray", "gray");
	var sqGrid2 = new THREE.GridHelper(floor_width, 25, "gray", "gray");


	sqGrid.position.set(0, - 1000, 0);
	sqGrid1.position.set(0, - 1000, -2000);
	sqGrid2.position.set(0, -1000, -4000);

	//sqGrid.rotation.y = 0 * THREE.MathUtils.DEG2RAD;
	//sqGrid1.rotation.y = 0 * THREE.MathUtils.DEG2RAD;
	//sqGrid2.rotation.y = 0 * THREE.MathUtils.DEG2RAD;
	...
	var planeGeometry = new THREE.PlaneGeometry(floor_width, floor_length, 100, 100);

	var planeMaterial = new THREE.MeshBasicMaterial({ color: "#7BB369", side: THREE.DoubleSide });
	...
	plane.rotation.z = 0 * THREE.MathUtils.DEG2RAD;

	scene.add(plane);


	// set the scene's background
	scene.background = new THREE.Color().setHex(0x82C8E5);
	...
	for (var i = 0; i < teapots.length; i++) {
		...
		/*Faster Shader Package
		// Replace custom RawShaderMaterial with built-in MeshPhongMaterial:
		var mesh = new THREE.Mesh(
			teapots[i].geometry,
			new THREE.MeshPhongMaterial({
				color: 0xffffff,             // you can adjust or pull from teapots[i].material
				specular: 0x222222,          // tweak as desired
				shininess: 30,               // lower shininess for speed
				flatShading: false,          // false = smooth normals
				side: THREE.DoubleSide,      // keep double-sided if you want
			})
		);*/
	...
	function updateUniforms(
		...
		for (var i = 0; i < teapots.length; i++) {
			...
			if (teapots[i].isHit) {
				// turn it bright red
				meshes[i].material.uniforms.material.value.diffuse.set(1, 0, 0);
			} //else {
				// revert to your normal diffuse color
				//meshes[i].material.uniforms.material.value.diffuse.copy(state.material.diffuse);
			//}
