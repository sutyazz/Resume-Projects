...
# -----------------------------
# Global Time Step for Simulation (in minutes)
# -----------------------------
time_step = 0.05 # 1 minute per frame
...
# -----------------------------
def bayesian_update(prior, new_data):
    """
    Simulate a Bayesian update for risk.
    For demonstration, we simply average the prior with the new sample mean.
    """
    updated = (prior + np.mean(new_data)) / 2.0
    return updated

def compute_wind_delay_correlation():
    """
    Compute and return the correlation between wind speed and flight delays.
    (This is a placeholder: real data would require matching weather and delay timestamps.)
    """
    if 'wind_speed' in weather_df.columns and 'Delay (min)' in traffic_df.columns:
        # For demonstration, use available wind speeds and delays
        wind = weather_df['wind_speed'].values
        delays = traffic_df['Delay (min)'].values
        if len(wind) > 0 and len(delays) > 0:
            corr = np.corrcoef(wind[:min(len(wind), len(delays))], delays[:min(len(wind), len(delays))])[0, 1]
            return corr
    return None

def compute_travel_time_stats(fp):
    """
    Compute expected travel time and a simple variance estimate.
    Here, variance is estimated as a fraction of the expected time.
    """
    expected = fp.metadata['expected_travel_time']
    variance = (0.05 * expected)**2  # 5% standard deviation
    return expected, variance

def dummy_ml_risk(features):
    """
    Placeholder for a probabilistic ML model that would predict risk.
    For now, returns a risk probability based on a simple weighted sum.
    """
    risk = 0.3 * features.get('weather_risk', 0) + 0.7 * features.get('traffic_risk', 0)
    return risk
...
def get_traffic_risk(planned_start_time, region="Palo Alto"):
    ...
    planned_hour = planned_start_time / 60.0  
...
class FlightPath:
    def __init__(self, flight_id, start_time, start_loc, dest_loc, cruising_altitude):
        ...
        self.times = None
        self.metadata = {}
        # Helicopter speed: 95 mph ≈ 2548 m/min
        self.speed = 2548  # m/min
        self.generate_path()
    ...
    def generate_path(self):
        ...
        mean_travel_time = total_distance / self.speed  # in minutes (expected travel time)
        self.times = np.linspace(self.start_time, self.start_time + mean_travel_time, total_steps)
        self.metadata['planned_path'] = self.path
        self.metadata['expected_travel_time'] = mean_travel_time
        self.metadata['sigma'] = 0.1 * mean_travel_time  # Set variance in travel time

    def sample_travel_time(self):
        mu = self.metadata['expected_travel_time']
        sigma = self.metadata['sigma']
        return np.random.normal(mu, sigma)

    def get_position_at_time(self, current_time):
        if current_time < self.start_time or current_time > self.times[-1]:
            return None  # Flight is not in the air at this time
        idx = np.searchsorted(self.times, current_time)
        idx = min(idx, len(self.path) - 1)
        return self.path[idx]


def compute_intersection_probability(fp1, fp2, num_samples=1000):
    safety_distance_threshold = 100  # meters, define as needed
    count = 0

    for _ in range(num_samples):
        # Sample travel times
        travel_time1 = fp1.sample_travel_time()
        travel_time2 = fp2.sample_travel_time()

        # Generate sampled times
        times1 = np.linspace(fp1.start_time, fp1.start_time + travel_time1, len(fp1.path))
        times2 = np.linspace(fp2.start_time, fp2.start_time + travel_time2, len(fp2.path))

        # Find overlapping time window
        start_time = max(fp1.start_time, fp2.start_time)
        end_time = min(times1[-1], times2[-1])
        if start_time >= end_time:
            continue  # No temporal overlap

        # Sample a random time within the overlap
        t = np.random.uniform(start_time, end_time)

        # Get positions at time t
        pos1 = fp1.get_position_at_time(t)
        pos2 = fp2.get_position_at_time(t)
...
def optimize_user_flight(user_fp, other_flights):
    """
    Adjust the user's flight start time and cruising altitude to minimize intersection probability.
    """
    start_time_shifts = np.arange(-10, 11, 2)  # from -10 to +10 minutes
    altitude_shifts = np.arange(-30, 31, 10)   # from -30 to +30 meters

    best_start_time = user_fp.start_time
    best_altitude = user_fp.cruising_altitude
    min_total_probability = float('inf')

    for time_shift in start_time_shifts:
        for alt_shift in altitude_shifts:
            # Adjust parameters
            temp_start_time = user_fp.start_time + time_shift
            temp_altitude = user_fp.cruising_altitude + alt_shift

            # Temporarily update flight parameters
            user_fp.start_time = temp_start_time
            user_fp.cruising_altitude = temp_altitude
            user_fp.generate_path()

            # Compute total intersection probability
            total_probability = 0
            for other_fp in other_flights:
                prob = compute_intersection_probability(user_fp, other_fp)
                total_probability += prob

            # Update best parameters if this is better
            if total_probability < min_total_probability:
                min_total_probability = total_probability
                best_start_time = temp_start_time
                best_altitude = temp_altitude

    # Set user's flight to best parameters found
    user_fp.start_time = best_start_time
    user_fp.cruising_altitude = best_altitude
    user_fp.generate_path()
    print(f"Optimized user's flight:")
    print(f"  Start Time: {best_start_time:.2f} minutes")
    print(f"  Cruising Altitude: {best_altitude} meters")
    print(f"  Minimized Intersection Probability: {min_total_probability:.4f}")
...
# -----------------------------
# Functions to Simulate Dynamic Updates (Placeholders) & Bayesian Updating
# -----------------------------
...
def dynamic_updates(flight_paths):
    update_time = 50
    # Simulate new weather data arriving and update risk using Bayesian updating.
    new_wind_data = np.random.uniform(0, 10, 100)
    for fp in flight_paths:
        if fp.start_time < update_time < fp.start_time + fp.metadata['expected_travel_time']:
            # Update weather risk using Bayesian update (placeholder)
            prior_risk, _ = get_weather_risk(fp.metadata['expected_travel_time'])
            updated_risk = bayesian_update(prior_risk, new_wind_data)
            fp.metadata['weather_risk'] = updated_risk
            fp.generate_path()
            print(f"Dynamic update applied to Flight {fp.flight_id}: Updated weather risk to {updated_risk:.2f}")
...
# -----------------------------
def get_active_traffic_flights(user_start_time, trip_duration):
    """
    Returns traffic flights that are either in progress or starting before the user's
    estimated trip (user_start_time + trip_duration) finishes.
    """
    active_flights = []
    for idx, row in traffic_df.iterrows():
        flight_start = time_str_to_float(row["Time of Travel"]) * 60  # convert to minutes
        duration = row["Duration (min)"]  # minutes
        flight_end = flight_start + duration
        if flight_end >= user_start_time and flight_start <= user_start_time + trip_duration:
            ...
            # Set variance
            fp.metadata['sigma'] = 0.1 * fp.metadata['expected_travel_time']  # Adjust as needed
...
# -----------------------------
# Emergency Rerouting Function
# -----------------------------
def emergency_reroute(flight_paths, emergency_fp):
    """
    In emergency mode, the user's flight (emergency_fp) is prioritized.
    All other flights are rerouted to avoid the corridor around the emergency flight.
    For demonstration, we simply raise the cruising altitude of other flights.
    """
    print("Emergency mode activated: Rerouting other flights.")
    for fp in flight_paths:
        if fp.flight_id != emergency_fp.flight_id:
            # Increase cruising altitude by an extra safety margin
            fp.cruising_altitude = max(fp.cruising_altitude, emergency_fp.cruising_altitude + 50)
            fp.generate_path()
            print(f"Rerouted Flight {fp.flight_id} to cruising altitude {fp.cruising_altitude}")
...
def visualize_flight_paths(flight_paths, total_simulation_time):
    ...
    ax.legend()
    plt.title('Optimized Flight Paths with Traffic, Collision Avoidance, and Emergency Rerouting')
    
    # Shift simulation to start at the minimum start time among flights
    simulation_start_time = min(fp.start_time for fp in flight_paths)

    def update(frame):
        current_time = simulation_start_time + frame * time_step
        for i, fp in enumerate(flight_paths):
            position = fp.get_position_at_time(current_time)
            if position is not None:
                ...
                # Update point
                points[i].set_data([position[0]], [position[1]])
                points[i].set_3d_properties([position[2]])
            else:
                lines[i].set_data([], [])
...
def user_input():
    ...
    # User inputs start time in hours; convert to minutes.
    start_time = float(input("\nEnter desired start time (in hours, e.g., 8.5 for 8:30 AM): ")) * 60
    chosen_start = starting_locations[start_choice]
    chosen_dest = destinations[dest_choice]
    
    # Emergency mode prompt
    emergency_input = input("\nAre you in an emergency? (y/n): ").strip().lower()
    emergency_mode = (emergency_input == 'y')
    
    return chosen_start, chosen_dest, start_time, emergency_mode

# -----------------------------
# Main Simulation Function
# -----------------------------
def simulate():
    chosen_start, chosen_dest, user_start_time, emergency_mode = user_input()
    ...
    exp_time, var_time = compute_travel_time_stats(optimized_fp)
    eta = optimized_fp.start_time + exp_time
    print(f"  Estimated Time of Arrival (ETA): {exp_time:.2f} minutes after {optimized_fp.start_time/60:.2f}")
    print(f"  Variance in Arrival Time: {var_time:.2f} (minutes^2)")
    weather_risk, _ = get_weather_risk(optimized_fp.metadata['expected_travel_time'])
    traffic_risk = get_traffic_risk(user_start_time)
    combined_risk = optimized_fp.metadata.get('combined_risk', 0)
    print(f"  Weather Risk: {weather_risk:.2f}")
    print(f"  Traffic Risk: {traffic_risk:.2f}")
    print(f"  Combined Risk: {combined_risk:.2f}")
    
    
    # Print correlation between wind speed and delays (if available)
    #corr = compute_wind_delay_correlation()
    #if corr is not None:
    #    print(f"  Correlation between wind speed and delays: {corr:.2f}")

    print("computing...")
    
   # Generate traffic flights
    trip_duration = optimized_fp.metadata['expected_travel_time']
    traffic_flights = get_active_traffic_flights(user_start_time, trip_duration)
    all_flights = [optimized_fp] + traffic_flights

    # Optimize user's flight
    optimize_user_flight(optimized_fp, traffic_flights)

    # Update estimated arrival time after optimization
    exp_time, var_time = compute_travel_time_stats(optimized_fp)
    eta = optimized_fp.start_time + exp_time
    print(f"\nUpdated Estimated Time of Arrival (ETA): {exp_time:.2f} minutes after {optimized_fp.start_time/60:.2f}")
    print(f"Variance in Arrival Time: {var_time:.2f} (minutes^2)")
    ...
    print(f"\nFound {len(traffic_flights)} active traffic flights (in progress or starting within your trip duration).")
    ...
    # If the user is in emergency, reroute other flights.
    if emergency_mode:
        emergency_reroute(all_flights, optimized_fp)
    
    dynamic_updates(all_flights)
    adjust_paths(all_flights)
    avoid_collisions(all_flights)
    
    total_calculated = len(all_flights)
    num_to_plot = int(input(f"Enter the number of flight paths to plot (max {total_calculated}): "))
    if num_to_plot > total_calculated:
        num_to_plot = total_calculated
    flights_to_plot = all_flights[:num_to_plot]
    
    total_simulation_time = max(fp.times[-1] for fp in flights_to_plot) + 10
    visualize_flight_paths(flights_to_plot, total_simulation_time)
